...
class UserController extends AbstractActionController {
    ...
    public function addAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                return $this->redirect()->toRoute('admin/users', ['action' => 'view', 'id' => $user->getId()]);
            }
        }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if ($user != null) {
                    ...
                    return $this->redirect()->toRoute('admin/users', ['action' => 'message', 'id' => 'sent']);
                } else {
                    return $this->redirect()->toRoute('adminusers', ['action' => 'message', 'id' => 'invalid-email']);
                }
            }
        }
    ...
    public function setPasswordAction() {
        ...
        if ($token === null || !$user) {
            if($token === null){
                $this->flashMessenger()->addErrorMessage(
                    'In order to recover password you need to fill the form.'
                );
            }else{
                $this->flashMessenger()->addInfoMessage(
                    'The token used is probably already expired, please try again.'
                );
            }
            
            return $this->redirect()->toRoute('admin/reset-password', ['action' => 'message', 'id' => 'failed']);
        }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if ($this->userManager->setNewPasswordByToken($token, $data['new_password'])) {
                    ...
                    // Redirect to "message" page
                    return $this->redirect()->toRoute('admin/users', ['action' => 'message', 'id' => 'set']);
                } else {
                    // Redirect to "message" page
                    return $this->redirect()->toRoute('admin/users', ['action' => 'message', 'id' => 'failed']);
                }
            }
        }
        return new ViewModel([
